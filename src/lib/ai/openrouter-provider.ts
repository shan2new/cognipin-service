 ...
 * 
 * Features:
 * - 128K token context window for extensive company information processing
 * - Built-in web search via :online model suffix
 * - Agentic capabilities for autonomous problem-solving
 * - Enhanced tool integration for sophisticated company searches
 */
export class OpenRouterProvider implements AIProvider {
  ...
  async searchCompanies(query: string): Promise<CompanySearchResponse> {
    const prompt = [
      "You are a research assistant with web search enabled. Use the web search results to find accurate, up-to-date company information.",
      "Task: Given the input string, identify companies explicitly or implicitly referenced (including prefix matches).",
      "Web Search Strategy: Search for company information from reliable business sources like Crunchbase, LinkedIn company pages, official websites, SEC filings, and business news.",
      ...
      "Data sources: Use web search to find information from Traxcn, Crunchbase, the company's official website, LinkedIn company pages, and reputable business news sources.",
    ...
    try {
      ...
      if (!outputText) {
        throw new Error("Failed to extract model output from OpenRouter");
      }
      ...
      const reformatResponse = await this.client.chat.completions.create({
        model: 'moonshotai/kimi-k2:free', // No web search needed for JSON formatting
        messages: [
          { 
            role: 'system', 
            content: [
          ...
          { role: 'user', content: this.buildSchemaInstruction() },
          { role: 'user', content: outputText }
        ],
        temperature: 0,
        max_tokens: 4000,
      ...
      const reformattedText = reformatResponse.choices[0]?.message?.content || '';
      const secondParse = this.strictParseCompanies(reformattedText, query);
      if (secondParse) {
        return secondParse;
      }

      console.error(`OpenRouter model did not return valid JSON for query "${query}" after retry. Raw outputs:`, { outputText, reformattedText });
      return { companies: [] };

    } catch (error) {
      console.error(`OpenRouter API error for query "${query}":`, error);
      throw error;
    }
  }
  ...
  private tryNormalizeParsedJSON(jsonText: string, query: string): CompanySearchResponse | null {
    try {
      ...
      if (parsed.name && parsed.websiteUrl) {
        console.warn(`OpenRouter model returned single company object instead of companies array for query "${query}". Converting to array format.`);
